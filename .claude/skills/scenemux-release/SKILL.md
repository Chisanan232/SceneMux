...
`SUPublicEDKey` are absent from the built `Info.plist`, and that the app launches. No `appcast.xml` is
produced: `APPCAST` defaults to `0` because SceneMux publishes no update feed, and turning it on is
the human-authorized decision listed at the end of this skill.

If no signing identity exists on the machine, use the ad-hoc invocation
